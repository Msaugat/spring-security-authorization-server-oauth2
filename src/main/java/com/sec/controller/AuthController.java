...
public class AuthController {
        ...
        try {
            ...
            if (oauthResponse.getStatusCode().is2xxSuccessful() && oauthResponse.getBody() != null) {
                ...
                return ResponseEntity.ok(Map.of(
                        ...
                        "scope", oauthResponse.getBody().get("scope"),
    ...
    public ResponseEntity<Map<String, Object>> refreshToken(@Valid @RequestBody RefreshTokenRequest request) {
        ...
        try {
            //  Use FORM PARAMETERS (same as login!) - NOT Basic Auth!
            MultiValueMap<String, String> formData = new LinkedMultiValueMap<>();
            ...
            log.info("Calling OAuth2 token endpoint with REFRESH TOKEN (form params)...");
            ...
            if (response.getStatusCode().is2xxSuccessful() && response.getBody() != null) {
                ...
                return ResponseEntity.status(response.getStatusCode())
                        .body(Map.of("error", "oauth2_error", "details", response.getBody()));
            }

        } catch (org.springframework.web.client.HttpClientErrorException.Unauthorized e) {
            log.error(" Refresh failed (401): {}", e.getResponseBodyAsString());

            return ResponseEntity.status(HttpStatus.UNAUTHORIZED)
                    .body(Map.of("error", "server_error", "message", e.getResponseBodyAsString()));
